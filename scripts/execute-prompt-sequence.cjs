...
**NOTA**: Este archivo se actualizará automáticamente durante la ejecución y se cerrará al completar el prompt ${promptNumber}.`;
  ...
  async executeAllPrompts() {
    console.log('🎯 INICIANDO GENERACIÓN DE ARCHIVOS DE CHAT PARA PROMPTS AWS');
    ...
    for (let i = 1; i <= this.totalPrompts; i++) {
      try {
        const filename = await this.executePrompt(i);
        generatedFiles.push(filename);
      } catch (error) {
        console.error(`❌ Error generando Prompt ${i}:`, error.message);
        break;
      }
    }
    ...
    console.log('1. Abrir un nuevo chat para cada prompt');
    console.log('2. Consultar el archivo correspondiente para instrucciones');
    console.log('3. Ejecutar todas las tareas del prompt');
    console.log('4. Documentar resultados en el mismo archivo');
    console.log('5. Continuar con el siguiente prompt solo si el anterior fue exitoso');
