...
class CRMService {
  ...
  async registrarCliente(clienteData) {
    try {
      ...
      const result = await this.sendToWebhookWithRetry('registrar-cliente', cliente)
  ...
  async registrarCompra(compraData) {
    try {
      ...
      const result = await this.sendToWebhookWithRetry('registrar-compra', compra)
  ...
  async programarSesion(sesionData) {
    try {
      ...
      const result = await this.sendToWebhookWithRetry('programar-sesion', sesion)
  ...
  async testConnection() {
    try {
      ...
      const result = await this.sendToWebhookWithRetry('test-connection', testData)
      return { 
        status: 'success', 
